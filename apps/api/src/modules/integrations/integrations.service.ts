...
export class IntegrationsService {
  ...
  getGraphQlInfo() {
    return {
      status: 'planned',
      endpoint: `${process.env.API_BASE_URL ?? 'https://api.creatorlend.com'}/graphql`,
      schema: null,
      playground: `${process.env.API_BASE_URL ?? 'https://api.creatorlend.com'}/graphql/playground`,
      library: '@nestjs/graphql + Apollo Server',
      estimatedRelease: 'v2.0',
      note: 'GraphQL is planned as an alternative to REST for flexible clients',
    };
  }

  // F-874: gRPC-Endpoints für interne Dienste
  getGrpcInfo() {
    return {
      status: 'planned',
      port: 5001,
      protoFiles: ['media.proto', 'notifications.proto', 'payouts.proto'],
      library: '@nestjs/microservices with grpc transport',
      useCases: ['Media service', 'Notification delivery', 'Payout processing'],
      tls: true,
    };
  }

  // F-878: OpenAPI/Swagger-Dokumentation
  getOpenApiInfo() {
    const base = process.env.API_BASE_URL ?? 'https://api.creatorlend.com';
    return {
      enabled: process.env.NODE_ENV !== 'production',
      swaggerUiUrl: `${base}/api/docs`,
      openApiJsonUrl: `${base}/api/docs-json`,
      version: 'OpenAPI 3.0',
      autoGenerated: true,
      library: '@nestjs/swagger',
      note: 'Enable SWAGGER_ENABLED=true to expose docs in production',
    };
  }
  ...
  getSdkInfo() {
    return {
      sdks: [
        { language: 'JavaScript/TypeScript', status: 'planned', generator: 'openapi-generator-cli', packageName: '@creatorlend/sdk' },
        { language: 'Python', status: 'planned', generator: 'openapi-generator-cli', packageName: 'creatorlend-python' },
        { language: 'Ruby', status: 'planned', generator: 'openapi-generator-cli', packageName: 'creatorlend-ruby' },
        { language: 'Go', status: 'planned', generator: 'openapi-generator-cli', packageName: 'creatorlend-go' },
      ],
      generationCommand: 'npx openapi-generator-cli generate -i /api/docs-json -g typescript-axios -o sdk/',
    };
  }

  // F-891: Batch-Actions (Bulk-Operationen)
  getBatchActionsInfo() {
    return {
      endpoints: [
        { method: 'POST', path: '/api/v1/works/batch', description: 'Bulk update work metadata', maxItems: 100 },
        { method: 'POST', path: '/api/v1/loans/batch', description: 'Bulk loan status update', maxItems: 50 },
        { method: 'POST', path: '/api/v1/users/batch', description: 'Admin: bulk user operations', maxItems: 200 },
      ],
      rateLimiting: '10 req/min for batch endpoints',
      idempotencyKey: 'X-Idempotency-Key header supported',
      status: 'planned',
    };
  }
  ...
  getExportInfo() {
    return {
      ...
      endpoints: [
        { path: '/api/v1/admin/exports/users', format: 'CSV', description: 'Export user base' },
        { path: '/api/v1/admin/exports/loans', format: 'CSV', description: 'Export loan history' },
        { path: '/api/v1/admin/exports/payouts', format: 'CSV', description: 'Export payout data' },
      ],
      asyncExport: true,
  ...
  // F-894: API Search & Filter (Cursor-basiert)
  getSearchFilterInfo() {
    return {
      paginationStyle: 'cursor-based',
      filterSyntax: 'field:value,field2:gt:value2',
      sortSyntax: 'sort=field:asc,field2:desc',
      globalSearchEndpoint: '/api/v1/search?q=',
      operators: ['eq', 'ne', 'gt', 'gte', 'lt', 'lte', 'in', 'contains'],
      fullTextSearch: { enabled: true, provider: 'PostgreSQL pg_trgm / pg_fts' },
    };
  }

  // F-896-F-897: API Authentication Enhancements
  getApiAuthInfo() {
    return {
      jwtRefreshTokens: { enabled: true, expiresIn: '7d', rotationOnUse: true },
      apiKeys: { enabled: true, scopes: ['read', 'write', 'admin'], rateLimit: '100/min free, 1000/min paid' },
      oAuth2: { enabled: true, flows: ['authorization_code', 'client_credentials'], pkce: true },
      mfa: { enabled: true, methods: ['TOTP', 'WebAuthn'], enforceForAdmin: true },
      sessionManagement: { maxActiveSessions: 5, deviceTracking: true },
    };
  }

  // F-899: Webhook-Management-UI
  getWebhookManagementInfo() {
    const base = process.env.API_BASE_URL ?? 'https://api.creatorlend.com';
    return {
      dashboardUrl: `${base}/api/v1/webhooks`,
      features: ['Delivery logs', 'Re-send failed events', 'Pause/resume endpoint', 'Test payload delivery'],
      retryPolicy: 'Exponential backoff: 1s, 2s, 4s, 8s, 16s (max 5 attempts)',
      signingSecret: 'X-CreatorLend-Signature header with HMAC-SHA256',
      eventTypes: ['loan.created', 'loan.expired', 'subscription.created', 'work.published', 'payout.completed'],
    };
  }
